...
export default function AidTab(props: AidTabProps) {
  const { isConnected, isMockWallet, address, isLoading, createCampaign, fundCampaign, approveMerchant, createCase, issueVoucher, redeemVoucher, appendEvidenceRevision, freezeClaim, decideClaim, addAidTrustline } = props;
  ...
  return (
    <section className="space-y-6" data-testid="aid-tab-root">
      ...
      <div className="p-5 rounded-2xl glass-card text-left space-y-4">
        ...
        <div className="rounded-xl border border-teal-500/15 bg-teal-950/10 px-3 py-2.5 text-xs leading-relaxed text-slate-400">Live Testnet requires authorized role wallets: admin and verifier authorities must be separate, and a verifier cannot approve their own merchant claim.</div>
      </div>

      {(isLoading || pending || (mode === "live" && (!isConnected || isMockWallet))) && <div aria-live="polite" className={`rounded-xl border px-4 py-3 text-xs ${mode === "live" && (!isConnected || isMockWallet) ? "border-amber-500/25 bg-amber-500/5 text-amber-200" : "border-teal-500/20 bg-teal-500/5 text-teal-200"}`}>{isLoading ? "Loading wallet state…" : pending ? `Pending submission: ${pending}…` : "Connect a real Testnet role wallet before submitting."}</div>}
